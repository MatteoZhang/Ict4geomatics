...
for i = 1:4
    ...
    lamda(i)  = atan(XYZ(i,2)/XYZ(i,1)); %radianti
    r = sqrt(XYZ(i,1)^2+XYZ(i,2)^2); %metri
    phi_old = atan(XYZ(i,3)/r); %radianti
    W = sqrt(1-e2(7)*sin(phi_old)^2);
    N = a(7)/W;
    h_old = (XYZ(i,1)/(cos(phi_old)*cos(lamda(i))))-N;
    phi_new = atan(XYZ(i,3)/(r*(1-(e2(7)*N)/(N+h_old))));
    err_h = abs(h_old);
    err_phi = abs(phi_new-phi_old);
    count = 0;%ma perchè h e phi diminuiscono anzichè aumentare?
    while((err_h>treshold) || (err_phi>treshold))
        W = sqrt(1-e2(7)*sin(phi_new)^2);
        N = a(7)/W;
        h_new = XYZ(i,1)/(cos(phi_old)*cos(lamda(i)))-N;
        phi_new = atan(XYZ(i,3)/(r*(1-(e2(7)*N)/(N+h_new))));
...
for i = 1:4
    ...
    while((err_h>treshold) || (err_phi>treshold))
        ...
        N = a(9)/W;
        h_new = XYZ(i,1)/(cos(phi_old)*cos(lamda(i)))-N;
        phi_new = atan(XYZ(i,3)/(r*(1-(e2(9)*N)/(N+h_new))));
        err_h = abs(h_old-h_new);
        err_phi = abs(phi_old-phi_new);
        h_old = h_new;
        phi_old = phi_new;
        count = count+1;
        h_3__w(i,count) = h_old;
        phi_3__w(i,count) = phi_old;
    end
end